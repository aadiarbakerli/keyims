  ...
  ngOnInit() {
    ...
    getKey(): void {
        ...
        if (this.curruser != null && this.curruser.keys != null) {
            const keylistp = document.getElementById('keylstp');

            while (keylistp.hasChildNodes()) {
                keylistp.removeChild(keylistp.childNodes[0]);
            }

            const keysp = this.curruser.keys;
        ...
        this.http.get<any>('/keyims/keyserv')
            .subscribe((data: Array<any>) => {
                ...
                for (let i = 0; i < this.keys.length; i++) {
                    ...
                    list.innerHTML = this.keys[i].id + ': ' + this.keys[i].desc + '<br> Type: ' +
                                     this.keys[i].type + '<br> Material: ' + this.keys[i].material + '<br> Qty: ' +
                                     this.keys[i].quantity + '<br> Public: ' + this.keys[i].pub ;
    ...
    delete() {
        ...
        const keyid = document.getElementById('keyid').innerHTML;
    ...
    clear() {
        ...
        (<HTMLButtonElement>document.getElementById('del')).disabled = true;
        const keyimg = (<HTMLImageElement>document.getElementById('imgdisp')).src = '';
    }
    ...
    showInfo(e) {
        ...
        const matpass = document.getElementById('matpass');
        const keymat = (<HTMLInputElement>document.getElementById('keymat'));
        const keydesc = (<HTMLInputElement>document.getElementById('keydesc'));
        const keyqty = (<HTMLInputElement>document.getElementById('keyqty'));
        const keypub = (<HTMLSelectElement>document.getElementById('keypub'));
        const keytype = (<HTMLInputElement>document.getElementById('keytype'));
        const keyimg = (<HTMLImageElement>document.getElementById('imgdisp'));
    ...
    submit() {
        ...
        (<HTMLButtonElement>document.getElementById('del')).disabled = true;
        const keyid = document.getElementById('keyid').innerHTML;
        const keymat = (<HTMLInputElement>document.getElementById('keymat')).value;
        const keydesc = (<HTMLInputElement>document.getElementById('keydesc')).value;
        const keyqty = (<HTMLInputElement>document.getElementById('keyqty')).value;
        const keypub = (<HTMLSelectElement>document.getElementById('keypub'))
                        .options[(<HTMLSelectElement>document.getElementById('keypub')).selectedIndex].value;
        const keytype = (<HTMLInputElement>document.getElementById('keytype')).value;
        const keyimg = (<HTMLInputElement>document.getElementById('keyimg')).files[0];
        const imgurl = (<HTMLImageElement>document.getElementById('imgdisp')).src;
        ...
        if (keyid !== '') {
            ...
            if (keyimg != null) {
                ...
                this.http.post('/keyims/file', f).subscribe((response) => (
                    (<HTMLButtonElement>document.getElementById('sub')).disabled = false), () => {
                        (<HTMLButtonElement>document.getElementById('sub')).disabled = false;
                    });
                this.clear();
